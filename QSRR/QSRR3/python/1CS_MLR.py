# -*- coding: utf-8 -*-
"""
Created on Tue Apr  9 14:44:02 2024
...
#总结最好的分数和最合适的特征数
print('Best MAE: %.3f' % results.best_score_)#1.430
print('Best Config: %s' % results.best_params_)#31
#总结所有的特征数和它对应的MAE
...
#存储特征数和得分
resultmi.to_csv('result\特征数MAE\csmlr_1.csv',header=True,index=False);
...
scd=scd.iloc[0:31,:]
#储存得分
scd.to_csv('result\特征得分\csmlr_1.csv',header=True,index=False);
...
all1.to_csv('result\预测结果\csmlr_1.csv',header=True,index=False)

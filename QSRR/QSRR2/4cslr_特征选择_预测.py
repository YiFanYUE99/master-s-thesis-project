# -*- coding: utf-8 -*-
"""
Created on Thu Mar 21 00:43:29 2024
...
fs = SelectKBest(score_func=f_regression)#css
...
#总结最好的分数和最合适的特征数
print('Best MAE: %.3f' % results.best_score_)#1.353
print('Best Config: %s' % results.best_params_)#5
#总结所有的特征数和它对应的MAE
...
#存储特征数和得分
resultmi.to_csv('特征数_MAE\cslrfs_no.csv',header=True,index=False);
...
#储存得分
scd20.to_csv('fs_score\cslrscore.csv',header=True,index=False);
...
print('MAE: %.3f' % mae)#所有数据的MAE是1.115
#预测数据和实验数据合并
...
all1.to_csv('result\Predict_cslr.csv',header=True,index=False)

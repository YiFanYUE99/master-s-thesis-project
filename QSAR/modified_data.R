...
#存储数据:合并moderate和low之后
write.csv(data, file = "QSAR/table/data3_MDs_modified.csv", row.names = TRUE)
...
ggsave("QSAR/pic/Ef_modified.png",width=10, height=7,dpi=300)
...
ggplot(data, aes(x = Ecotoxicity,
  ...
  theme(text = element_text(family = "mono", size=14),
        ...
        panel.grid.minor = element_blank(),
        panel.border = element_rect(color = "grey", fill = NA))#保留边框
ggsave("QSAR/pic/Ecotoxicity_modified.png",width=10, height=7,dpi=300)
...
ggplot(data, aes(x = `Human health`,
  ...
  theme(text = element_text(family = "mono", size=14),
        ...
        panel.grid.minor = element_blank(),
        panel.border = element_rect(color = "grey", fill = NA))#保留边框
ggsave("QSAR/pic/HH_modified.png",width=10, height=7,dpi=300)

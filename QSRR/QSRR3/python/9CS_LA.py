# -*- coding: utf-8 -*-
"""
Created on Tue Apr  9 19:20:23 2024
...
#总结最好的分数和最合适的特征数
print('Best MAE: %.3f' % results.best_score_)#1.559
print('Best Config: %s' % results.best_params_)#114
#总结所有的特征数和它对应的MAE
...
#存储特征数和得分
resultmi.to_csv('result\特征数MAE\csla_9.csv',header=True,index=False);
...
def select_features(X_train, y_train, X_test):
	...
	fs = SelectKBest(score_func=f_regression, k=114)
...
scd=scd.iloc[0:114,:]
#储存得分
scd.to_csv('result\特征得分\csla_9.csv',header=True,index=False);
...
all1.to_csv('result\预测结果\csla_9.csv',header=True,index=False)

# -*- coding: utf-8 -*-
"""
Created on Thu Apr 11 19:23:10 2024
...
# 输出最优参数
print("Best number of features:", grid_search.best_params_['fs__k'])#118
print('Best AUC: %.3f' % grid_search.best_score_)#0.543
...
fi.to_csv('特征重要性/EF_fGB_fimp_6.csv',header=True, index = True)
...
print("在训练集上的正确率:", test_accuracy)#0.559
train_accuracy = model.score(X_train, y_train)
print("在训练集上的正确率:", train_accuracy)#0.945
full_set_accuracy = model.score(X, y)
print("在全集上的正确率:", full_set_accuracy)#0.829
...
EF_RF_ROC.to_csv('ROC曲线/EF_fGB_ROC_6.csv',header=True,index=False);  
...
confmat.to_csv('混淆矩阵/EF_fGB_confmat_6.csv',header=True, index = True)

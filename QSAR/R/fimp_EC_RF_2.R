...
xtesize<-14
ytesize<-10
fimp<-read.csv("QSAR/特征重要性/EC_fRF_fimp_7.csv",header = TRUE,check.names = FALSE)
colnames(fimp)=c("Name","score")
ECRF<-ggplot(fimp, aes(x = Name, y = score)) +
  geom_segment(aes(x= Name, y= score, color=score,
                   xend=Name, yend=12,linewidth=score/linesize),
               linetype=1)+#使用reorder()排序变量
  geom_point(aes(fill=score, size=score/pointsize), pch = 21, color=EC[1]) +
  ...
  labs(title = "Ecotoxicity & Random Forest AUC=0.934", x = "Molecular Descriptors", y = "Score")+
...
ggsave(plot = ECRF,"QSAR/pic2/feature_imp_EC.png",width = 20,height = 20,dpi = 300,units = "cm")

# -*- coding: utf-8 -*-
"""
Created on Thu Apr 11 22:52:17 2024
...
print('Best AUC: %.3f' % grid_search.best_score_)#0.620
...
fi.to_csv('特征重要性/EC_fNB_fimp_10.csv',header=True, index = True)


# 在测试集上评估模型
y_pred_proba = model.predict_proba(X_test)[:, 1]
auc = roc_auc_score(y_test, y_pred_proba)
print("AUC: %.3f" %  auc)#0.561
#训练集上评估模型
y_pred_proba = model.predict_proba(X_train)[:, 1]
auc = roc_auc_score(y_train, y_pred_proba)
print("AUC: %.3f" %  auc)#0.663
#全集上评估模型
y_pred_proba = model.predict_proba(X)[:, 1]
auc = roc_auc_score(y, y_pred_proba)
print("AUC %.3f" % auc)#0.631

# 计算在训练集上的正确率
test_accuracy = model.score(X_test, y_test)
print("在训练集上的正确率: %.3f" %  test_accuracy)#0.538
train_accuracy = model.score(X_train, y_train)
print("在训练集上的正确率: %.3f" %  train_accuracy)#0.567
full_set_accuracy = model.score(X, y)
print("在全集上的正确率: %.3f" %  full_set_accuracy)#0.558
...
EF_RF_ROC.to_csv('ROC曲线/EC_fNB_ROC_10.csv',header=True,index=False);  
...
confmat.to_csv('混淆矩阵/EC_fNB_confmat_10.csv',header=True, index = True)

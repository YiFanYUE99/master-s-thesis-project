# -*- coding: utf-8 -*-
"""
Created on Thu Mar 21 15:29:14 2024
...
fs = SelectKBest(score_func=f_regression)#cs
...
#总结最好的分数和最合适的特征数
print('Best MAE: %.3f' % results.best_score_)#1.203
print('Best Config: %s' % results.best_params_)#55
#总结所有的特征数和它对应的MAE
...
#存储特征数和得分
resultmi.to_csv('特征数_MAE\cslafs_no.csv',header=True,index=False);

#输出最优特征数的score
from sklearn.feature_selection import SelectKBest, f_regression
def select_features(X_train, y_train, X_test):
	# configure to select a subset of features
	fs = SelectKBest(score_func=f_regression, k=55)
...
#降序排列后取前20个
scd20=scd1.sort_values(by='Score', ascending=False).iloc[0:55,:]
#储存得分
scd20.to_csv('fs_score\cslascore.csv',header=True,index=False);
...
all1.to_csv('result\Predict_csla.csv',header=True,index=False)

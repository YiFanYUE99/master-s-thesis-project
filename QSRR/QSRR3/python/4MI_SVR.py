# -*- coding: utf-8 -*-
"""
Created on Tue Apr  9 16:30:57 2024
...
#总结最好的分数和最合适的特征数
print('Best MAE: %.3f' % results.best_score_)#1.523
print('Best Config: %s' % results.best_params_)#1
#总结所有的特征数和它对应的MAE
...
#存储特征数和得分
resultmi.to_csv('result\特征数MAE\misvr_4.csv',header=True,index=False);

#输出最优特征数的score
from sklearn.feature_selection import SelectKBest, mutual_info_regression
def select_features(X_train, y_train, X_test):
	# configure to select a subset of features
	fs = SelectKBest(score_func=mutual_info_regression, k=1)
...
scd=scd.iloc[0:1,:]
#储存得分
scd.to_csv('result\特征得分\misvr_4.csv',header=True,index=False);
...
all1.to_csv('result\预测结果\misvr_4.csv',header=True,index=False)

# -*- coding: utf-8 -*-
"""
Created on Thu Apr 11 23:20:54 2024
...
pipeline = Pipeline([
    ...
    ('svm', SVC(kernel='rbf', probability=True,C=1,random_state=707))
])

# 定义参数网格
param_grid = {
    'fs__k': range(1, len(X_train.columns) + 1)  # 尝试不同的特征数
}
...
# 输出最优特征数
best_n_features = grid_search.best_params_['fs__k']
print("最优特征数:", best_n_features)#148
print('Best AUC: %.3f' % grid_search.best_score_)#0.579
...
fi.to_csv('特征重要性/HH_fSVM_fimp_14.csv',header=True, index = True)
...
print("AUC:%.3f" % auc)#0.544
#训练集上评估模型
y_pred_proba = model.predict_proba(X_train)[:, 1]
auc = roc_auc_score(y_train, y_pred_proba)
print("AUC:%.3f" % auc)#0.864
#全集上评估模型
y_pred_proba = model.predict_proba(X)[:, 1]
auc = roc_auc_score(y, y_pred_proba)
print("AUC:%.3f" % auc)#0.777


# 计算在训练集上的正确率
test_accuracy = model.score(X_test, y_test)
print("在测试集上的正确率:%.3f" % test_accuracy)#0.563
train_accuracy = model.score(X_train, y_train)
print("在训练集上的正确率:%.3f" % train_accuracy)#0.772
full_set_accuracy = model.score(X, y)
print("在全集上的正确率:%.3f" % full_set_accuracy)#0.709
...
EF_RF_ROC.to_csv('ROC曲线/HH_fSVM_ROC_14.csv',header=True,index=False);  
...
confmat.to_csv('混淆矩阵/HH_fSVM_confmat_14.csv',header=True, index = True)

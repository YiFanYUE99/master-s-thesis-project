...
fimp<-read.csv("QSAR/table/data25EC_LR_fimp.csv",header = TRUE,check.names = FALSE)
colnames(fimp)=c("Name","abs(coef)")
EC<-c("#FF869D","#FFE0E6")
LR<-c("#86FF86","#E0FFE0")
ECLR<-ggplot(fimp, aes(x = Name, y = `abs(coef)`)) +
  ...
  guides(fill ="none",color="none")+
  labs(title = "Ecotoxicity & Logistic Regression", x = "", y = "Abs(coef)")+
  xlim(rev(fimp$Name))+#固定横坐标
  coord_flip()+
  theme(
    text = element_text(family = "mono", size=16),
...
EC<-c("#FF869D","#FFE0E6")
SVM<-c("#86FFFF","#E0FFFF")
ECSVM<-ggplot(fimp, aes(x = Name, y = `abs(coef)`)) +
  geom_segment(aes(x= Name, y=`abs(coef)`, color=`abs(coef)`,
                   xend=Name, yend=0.66),
               size=5,
               linetype=1)+#使用reorder()排序变量
  ...
  guides(fill ="none",color="none")+
  labs(title = "Ecotoxicity & SVM", x = "", y = "Abs(coef)")+
  xlim(rev(fimp$Name))+#固定横坐标
  coord_flip()+
  theme(
    text = element_text(family = "mono", size=16),
...
com<-grid.arrange(ECRF,ECLR,ECSVM,nrow=1)
save_plot("QSAR/final/pic/EC_fimp.png",com,base_width=50,base_height = 15,dpi = 300,units = "cm",bg="white")

# -*- coding: utf-8 -*-
"""
Created on Thu Apr 11 22:43:15 2024
...
# 输出最优参数
print("Best number of features:", grid_search.best_params_['fs__k'])#10
print('Best AUC: %.3f' % grid_search.best_score_)#0.663
...
fi.to_csv('特征重要性/EC_fLR_fimp_9.csv',header=True, index = True)

# 在测试集上评估模型
y_pred_proba = model.predict_proba(X_test)[:, 1]
auc = roc_auc_score(y_test, y_pred_proba)
print("AUC: %.3f" % auc)#0.537
#训练集上评估模型
y_pred_proba = model.predict_proba(X_train)[:, 1]
auc = roc_auc_score(y_train, y_pred_proba)
print("AUC: %.3f" % auc)#0.674
#全集上评估模型
y_pred_proba = model.predict_proba(X)[:, 1]
auc = roc_auc_score(y, y_pred_proba)
print("AUC: %.3f" % auc)#0.633

# 计算在训练集上的正确率
test_accuracy = model.score(X_test, y_test)
print("在训练集上的正确率: %.3f" % test_accuracy)#0.580
train_accuracy = model.score(X_train, y_train)
print("在训练集上的正确率: %.3f" % train_accuracy)#0.679
full_set_accuracy = model.score(X, y)
print("在全集上的正确率: %.3f" % full_set_accuracy)#0.650
...
EF_RF_ROC.to_csv('ROC曲线/EC_fLR_ROC_9.csv',header=True,index=False);  
...
confmat.to_csv('混淆矩阵/EC_fLR_confmat_9.csv',header=True, index = True)

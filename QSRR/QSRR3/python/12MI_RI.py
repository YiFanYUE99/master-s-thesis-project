# -*- coding: utf-8 -*-
"""
Created on Tue Apr  9 21:37:20 2024
...
pipeline = Pipeline(steps=[('sel', fs), ('ridge', model)])
#第一步是特征选择fs，然后是建模lr
...
#总结最好的分数和最合适的特征数
print('Best MAE: %.3f' % results.best_score_)#1.755
print('Best Config: %s' % results.best_params_)#3
#总结所有的特征数和它对应的MAE
...
#存储特征数和得分
resultmi.to_csv('result\特征数MAE\miri_12.csv',header=True,index=False);
...
scd=scd.iloc[0:3,:]
#储存得分
scd.to_csv('result\特征得分\miri_12.csv',header=True,index=False);
...
all1.to_csv('result\预测结果\miri_12.csv',header=True,index=False)

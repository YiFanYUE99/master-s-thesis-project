# -*- coding: utf-8 -*-
"""
Created on Tue Apr  9 16:03:09 2024
...
#总结最好的分数和最合适的特征数
print('Best MAE: %.3f' % results.best_score_)#1.751
...
#存储特征数和得分
resultmi.to_csv('result\特征数MAE\mimlr_2.csv',header=True,index=False);
...
print('MAE: %.3f' % mae)#所有数据的MAE是1.647
#可视化3个特征的得分
sc=[]
...
scd=scd.iloc[0:3,:]
#储存得分
scd.to_csv('result\特征得分\mimlr_2.csv',header=True,index=False);
...
all1.to_csv('result\预测结果\mimlr_2.csv',header=True,index=False)

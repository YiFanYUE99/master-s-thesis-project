# -*- coding: utf-8 -*-
"""
Created on Thu Apr 11 23:31:31 2024
...
# 输出最优参数
print("Best number of features:", grid_search.best_params_['fs__k'])#93
print('Best AUC: %.3f' % grid_search.best_score_)#0.564
...
fi.to_csv('特征重要性/HH_fKNN_fimp_17.csv',header=True, index = True)
...
print("AUC:%.3f" % auc)#0.800
#全集上评估模型
y_pred_proba = model.predict_proba(X)[:, 1]
auc = roc_auc_score(y, y_pred_proba)
print("AUC:%.3f" % auc)#0.726

# 计算在训练集上的正确率
test_accuracy = model.score(X_test, y_test)
print("在训练集上的正确率:%.3f" % test_accuracy)#0.542
train_accuracy = model.score(X_train, y_train)
print("在训练集上的正确率:%.3f" % train_accuracy)#0.738
full_set_accuracy = model.score(X, y)
print("在全集上的正确率:%.3f" % full_set_accuracy)#0.679
...
EF_RF_ROC.to_csv('ROC曲线/HH_fKNN_ROC_17.csv',header=True,index=False);  
...
confmat.to_csv('混淆矩阵/HH_fKNN_confmat_17.csv',header=True, index = True)
